...
import {PEERJS_HOST, PEERJS_PORT, PEERJS_SECURE, SOCKET_HOST} from "../constant";
...
const SocketContextProvider = ({children}) => {
  ...
  const sendPosition = (name, pos) => {
    socket.emit("position", name,JSON.stringify({
  ...
  useEffect(() => {
    const initSocket = () => {
      ...
      socket.on("position", (id, position) => {
        setNewPos( { [id]: JSON.parse(position)})
      });
